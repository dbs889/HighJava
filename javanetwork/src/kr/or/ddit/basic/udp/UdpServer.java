...
/*
 * UDP 방식 : 비 연결 지향 , 데이터의 신뢰성이 없다 데이터가 순서대로 도착한다는 보장이 없다
 * 			그렇지만 TCP방식보다 속도가 빠르다
 * 
 * DatagramSocket 객체와 DatagramPacket객체를 이용하여 통신한다
 * 
 * 1) DatagramSocket : 데이터의 송수긴과 관련된 작업을 수행한다(우체부역할)
 * 2) DatagramPacket : 주고 받는 데이터와 관련된 작업을 수행한다(소포,편지)
 * 						==> 수신을 위한 생성자와 송신을 위한 생성자를 따로 제공한다
 * 		TCP 방식의 경우에는 스트림을 이용해서 송수신 하지만 
	...
	public static void main(String[] args) {
		...
		try {
			...
			while(true){
				...
				//데이터가 저장될 byte[]형 배열 변수 선언
				byte[] bMsg = new byte[1024];
				
				
				//수신용 패킷객체 생성
				// 				==> 생성자에 데이터가 저장될 byte형 배열의 길이를 넣어서 생성한다
				
				inpacket = new DatagramPacket(bMsg, bMsg.length);
				
				//데이터를 수신한다 ==>receive() 메서드 사용
				//		이 메서드는 데이터가 올 때까지 기다린다
				// 		수신된 데이터의 패킷정보는 지정된 패킷변수에 저장된다
				socket.receive(inpacket);
				
				//수신받은 패킷에서 상대방의 주소,포트번호등을 알 수 임ㅆ다
				InetAddress address = inpacket.getAddress();
				int port = inpacket.getPort();
				
				
				System.out.println("상대방 IP정보 : " + address);
				System.out.println("상대방 port정보 : " + port);
				
				System.out.println();
				
				
				//상대방이 보낸 메시지 출력하기
				//수신용 패킷변수.getLength(); ==> 실제 일어온데이터의 길이를 반환한다
				//수신용 패킷 변수.getData(); ==> 실제 읽어온 데이터를 byte형 배열로 반환한다
				...
				//상대방에게 데이터 송신하기(수신된 데이터를 그대로 송신허눈 예제)
				...
				//		전송할 자료의 길이(배열의 길이), '상대방주소정보', '상대방 포트번호'
				//		위 4가지를 생성자에 넣어서 생성한다
				
				if("/end".equals(sendMsg)){ //'/end'메시지가 오면 프로그램 종료
					break;
				}
