...
//RMI기술을 제공하는 클래스 => RMI용 인터페이스를 구현하고 UnicastRemoteObject를 상속해서 작성한다
...
public class RemoteServer extends UnicastRemoteObject implements RemoteInterface{
	...
	public static void main(String[] args) {
		...
		try {
			...
			//		(이때의 변수는 RMI용 인터페이스를 자료형으로 선언한다)
			
			RemoteInterface server = new RemoteServer();
			
			
			//2. 구현한 객체를 클라이언트에서 찾을 수 있도록 관리하는 Registry
			...
			//3. Resisty에 서버에서 제공할 객체를 등록한다  (즉, RMI용 인터페이스를 구현한 객체의 인스턴스를 등록한다)
			// 	등록방법 ) Resistry 객체변수.rebind("객체의 Alias",RMI용 객체의 인스턴스);
			//		==> 등록할 때 지정한 '객쳉의 Alias'는 클라이언트에서 이 객체를 구할 때 사용하는 이름이 된다
	...
	public int doRemotePrint(String str) throws RemoteException {
		...
		System.out.println("클라이언트에서 보내 온 내용(=매개변수 값) : " + str);
		System.out.println("doRemotePrint()메서드 끝");
		System.out.println();
		
		
		return 200;  //이 반환값은 서버에서 클라이언트 쪽으로 보내는 값이 된다
	}
	...
	@Override //클라이언트에세 TestVo를 담아서 보낸다
